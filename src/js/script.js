...
//_____________________________Nice Select plugin______________________________
(function($) {
...
const changeSelectLocation = () => {
	$('#langSelect').niceSelect();
	if(window.innerWidth <= 650) {
		const $li = $('<li></li>');
		$('.nice-select').css({
			marginLeft: `${$('.top-menu__nav-menu').width() - $('.nice-select').outerWidth()}px`
		});
		$li.append($('.top-menu__languages:first'));
		$('.top-menu__nav-menu__item:last').after($li)
	}
};
//activate the plugin
	...
	for (let i = 0; i < elem.length; i++) {
		for (let i = 0; i < elem.length; i++) {
			elem[i].onclick = function () {
				...
				for (let i = 0; i < elemCont.length; i++) {
					elemCont[i].classList.remove("active-content");
				}
				elemCont[i].classList.toggle("active-content");
			};
		}
	}
};
				...
				$.ajax({
					...
					success: function () {
						$("#orderForm").animate({opacity: 0},500,() => {
							...
							$('.order__form-wrapper__succes').fadeIn(1500);
						});
					}
				});
			} else {
...
$("#nav-toggle").click(function(){
	...
	let backgroundColor = $('<div id="background"></div>');
	$(backgroundColor).height($(document).height());
	backgroundColor.insertBefore('.top-menu__nav-menu');
});
